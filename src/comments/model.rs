...
impl Comment {
    ...
    pub fn find_by_user(id:i64) -> Result<Vec<Self>, CustomError>{
        let conn = db::connection()?;
        let comments = comments::table.filter(comments::user_id.eq(id)).load::<Comment>(&conn)?;
        Ok(comments)
    }

    pub fn find_by_asset_tag(id:i64) -> Result<Vec<Self>, CustomError>{
        let conn = db::connection()?;
        let comments = comments::table.filter(comments::asset_tag_id.eq(id)).load::<Comment>(&conn)?;
        Ok(comments)
    }    
